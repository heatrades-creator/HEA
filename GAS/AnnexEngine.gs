...
function _buildSystemSpecPayload_(job, today) {
  ...
  return {
    ...
    // Panel fields — from Quote Builder equipment columns
    '{SS_PanelMake}':      job.panelBrand  || '',
    '{SS_PanelModel}':     job.panelModel  || '',
    '{SS_PanelWatts}':     '',
    '{SS_PanelCount}':     '',
    // Inverter fields
    '{SS_InverterType}':   '',
    '{SS_InverterMake}':   job.inverterBrand || '',
    '{SS_InverterModel}':  job.inverterModel || '',
    '{SS_InverterKw}':     '',
    // Battery
    '{SS_BatterySizeKwh}': job.batteryKwh ? job.batteryKwh + ' kWh' : (hasBattery ? job.batterySize + ' kWh' : 'No battery'),
    '{SS_BatteryMake}':    job.batteryBrand  || '',
    '{SS_BatteryModel}':   job.batteryModel  || '',
    '{SS_BatteryUsableKwh}': job.batteryKwh || '',
