...
export async function GET(req: Request) {
  ...
  const qs = new URLSearchParams({ action: 'listTasks', ...(status && { status }), ...(assignedTo && { assigned_to: assignedTo }), ...(entityId && { entity_id: entityId }) });
  try {
...
export async function POST(req: Request) {
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ action: 'createTask', actor: session.user?.email, ...body }),
    });
    return Response.json(await res.json());
  } catch {
    return Response.json({ error: 'GAS request failed' }, { status: 502 });
