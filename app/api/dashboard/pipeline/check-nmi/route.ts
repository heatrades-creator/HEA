...
export async function GET(req: NextRequest) {
  ...
  const raw = await fetch(`${gasUrl}?action=checkNMI&jobNumber=${encodeURIComponent(jobNumber)}`, { cache: 'no-store' })
  const text = await raw.text()
  try {
    const data = JSON.parse(text)
    if (data.error) return NextResponse.json({ hasNMI: false }, { status: 200 })
    return NextResponse.json(data)
  } catch {
    return NextResponse.json({ hasNMI: false })
  }
}
