...
export async function GET(req: NextRequest) {
  ...
  const raw = await fetch(`${gasUrl}?action=checkOpenSolar&jobNumber=${encodeURIComponent(jobNumber)}`, { cache: 'no-store' })
  const text = await raw.text()
  try {
    const data = JSON.parse(text)
    if (data.error) return NextResponse.json({ hasOpenSolar: false })
    return NextResponse.json(data)
  } catch {
    return NextResponse.json({ hasOpenSolar: false })
  }
}
