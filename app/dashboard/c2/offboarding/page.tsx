    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {active.length === 0 ? (
        <div className="bg-white border border-[#e5e9f0] rounded-xl p-8 text-center">
          <p className="text-[#9ca3af] text-sm">No active offboarding cases.</p>
        </div>
      ) : (
            ...
            <div key={c.case_id} className="bg-white border border-[#e5e9f0] rounded-xl p-5">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-[#9ca3af] text-xs mt-0.5">{(c.reason || '').replace(/_/g, ' ')}</p>
                  {c.last_day && <p className="text-[#6b7280] text-xs mt-1">Last day: <span className="text-[#aaa]">{String(c.last_day).substring(0, 10)}</span></p>}
                </div>
                ...
                {CHECKLIST.map(item => {
                  ...
                  return (
                    <div key={item.key} className="flex items-center gap-1.5">
                      ...
                      <span className={`text-xs ${done ? 'text-[#9ca3af] line-through' : 'text-[#6b7280]'}`}>{item.label}</span>
                    </div>
                  );
                })}
              </div>

              {c.notes && <p className="text-[#9ca3af] text-xs mt-3 border-t border-[#edf0f5] pt-3">{c.notes}</p>}
            </div>
          ))}
        </div>
      )}
