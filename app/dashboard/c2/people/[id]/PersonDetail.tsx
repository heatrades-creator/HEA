    ...
    <div className="bg-[#202020] rounded-xl border border-[#2e2e2e] overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-[#2e2e2e] flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[#888] text-sm mt-0.5">{(String(person.employment_type ?? '')).replace(/_/g, ' ')}</p>
        </div>
        <p className="text-[#555] text-xs whitespace-nowrap">{String(person.created_at ?? '').substring(0, 10)}</p>
      </div>
      ...
      <div className="p-6 space-y-7">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select value={employmentType} onChange={e => setEmploymentType(e.target.value)} className="w-full bg-[#2a2a2a] border border-[#333] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#ffd100]">
        ...
        <div>
          ...
          <p className="text-[#555] text-xs mb-3">Current: <span className="text-white">{String(person.status ?? '')}</span>. Select a new status — transitions are validated and audit-logged.</p>
          <div className="flex flex-wrap gap-2">
            {STATUSES.filter(s => s !== String(person.status ?? '').toUpperCase()).map(s => (
              <button key={s} onClick={() => transitionStatus(s)} className="px-3 py-1.5 border border-[#333] rounded-lg text-xs text-[#666] hover:border-[#555] hover:text-[#aaa] transition-colors">
                → {s.replace(/_/g, ' ')}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} className="w-full bg-[#2a2a2a] border border-[#333] rounded-lg px-3 py-2 text-white text-sm placeholder:text-[#444] focus:outline-none focus:border-[#ffd100] resize-none" />
        </div>
...
function TextInput({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      ...
      <input type="text" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full bg-[#2a2a2a] border border-[#333] rounded-lg px-3 py-2 text-white text-sm placeholder:text-[#444] focus:outline-none focus:border-[#ffd100]" />
    </div>
  );
}
