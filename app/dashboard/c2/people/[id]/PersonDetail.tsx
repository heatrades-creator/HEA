  ...
  return (
    <div className="bg-white rounded-xl border border-[#e5e9f0] overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-[#e5e9f0] flex items-start justify-between gap-4">
        ...
        <p className="text-[#9ca3af] text-xs whitespace-nowrap">{String(person.created_at ?? '').substring(0, 10)}</p>
      </div>
      ...
      <div className="p-6 space-y-7">
        ...
        <div>
          ...
          <p className="text-[#9ca3af] text-xs mb-3">Current: <span className="text-[#111827] font-medium">{String(person.status ?? '')}</span>. Select a new status — transitions are validated and audit-logged.</p>
        ...
        <div>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} className="w-full bg-[#eef0f5] border border-[#e5e9f0] rounded-lg px-3 py-2 text-[#111827] text-sm placeholder:text-[#9ca3af] focus:outline-none focus:border-[#ffd100] resize-none" />
        </div>
...
function TextInput({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return (
    <div>
      ...
      <input type="text" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full bg-[#eef0f5] border border-[#e5e9f0] rounded-lg px-3 py-2 text-[#111827] text-sm placeholder:text-[#9ca3af] focus:outline-none focus:border-[#ffd100]" />
    </div>
  );
}
