...
function computeVerdict(a: Answers): { verdict: Verdict; rationale: string; financialNote: string; recommendedKwh: string } {
  ...
  return {
    ...
    rationale:
      a.solar === "no_solar"
        ? "Get solar installed first — a battery's job is to store solar you'd otherwise export."
    ...
    financialNote: "Adding a battery before solar typically stretches payback past 12 years.",
  ...
  return (
    <button
      ...
      className={`w-full text-left px-5 py-4 rounded-xl border-2 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-yellow-400
        ${selected ? "border-yellow-400 bg-yellow-50" : "border-slate-200 hover:border-yellow-300 hover:bg-yellow-50/40"}`}
    >
...
interface BatteryQuizWidgetProps {
  ctaUrl: string;
  ctaLabel: string;
}

export default function BatteryQuizWidget({ ctaUrl, ctaLabel }: BatteryQuizWidgetProps) {
  ...
  const answer = <K extends keyof Answers>(key: K, value: Answers[K]) => {
    ...
    setAnswers(next);
    if (step < totalSteps - 1) { setStep(step + 1); } else { setDone(true); }
  };
  ...
  return (
    <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden font-sans">
      <div className="bg-slate-900 px-6 py-5">
        ...
        <p className="text-slate-400 text-sm mt-1">Answer 4 quick questions — get a straight yes, maybe, or not yet.</p>
      </div>
      ...
      <div className="p-6">
        {!done && (
          <div className="mb-6">
            ...
            <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
              <div className="h-full bg-yellow-400 rounded-full transition-all duration-300" style={{ width: `${((step + 1) / totalSteps) * 100}%` }} />
            </div>
          </div>
        )}
        ...
        {done && result && cfg && (
          <div>
            ...
            <p className="text-xs text-slate-400 italic mb-5">Indicative only — actual payback is modelled from your site and usage data.</p>
