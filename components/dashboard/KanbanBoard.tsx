...
export default function KanbanBoard({ initialJobs }: { initialJobs: Job[] }) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-57px)] -m-6">
      {/* ── Left sidebar ── */}
      <aside className="w-52 flex-shrink-0 bg-[#1a1a1a] border-r border-[#242424] p-5 flex flex-col gap-6">
        {/* Total count */}
        <div className="pt-1">
          <div className="text-4xl font-bold text-white tabular-nums leading-none">{jobs.length}</div>
          <div className="text-[#555] text-xs mt-1.5">Total Jobs</div>
        </div>
      ...
      <div className="flex-1 flex flex-col p-6 min-w-0">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          {/* View toggle */}
          <div className="flex bg-[#202020] border border-[#2e2e2e] rounded-lg overflow-hidden">
            <button
            ...
            <button
              ...
              title="Kanban view"
              className={`px-3 py-2 transition-colors text-sm border-l border-[#2e2e2e] ${
                viewMode === 'kanban'
                  ? 'bg-[#ffd100]/10 text-[#ffd100]'
                  : 'text-[#555] hover:text-[#aaa]'
              }`}
        ...
        return (
          <div key={stage} className="flex-shrink-0 w-72">
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-[#555] bg-[#222] px-2 py-0.5 rounded-full tabular-nums">
                {stageJobs.length}
              </span>
            </div>
...
function JobCard({
  ...
  return (
    <div
      className={`bg-[#202020] border ${styles.border} rounded-xl p-4 cursor-pointer hover:border-[#ffd100]/50 transition-all relative`}
      onClick={onClick}
    >
      ...
      <div className="mt-3 relative" onClick={(e) => e.stopPropagation()}>
        ...
        {showMove && (
          <>
            ...
            <div className="absolute left-0 top-6 bg-[#2a2a2a] border border-[#333] rounded-lg overflow-hidden z-20 w-40 shadow-xl">
              {STAGES.map((s) => (
                <button
                  key={s}
