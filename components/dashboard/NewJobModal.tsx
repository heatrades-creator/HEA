    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-[#202020] border border-[#2e2e2e] rounded-xl w-full max-w-lg">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2e2e2e]">
          <h2 className="text-white font-semibold">New Job</h2>
          <button onClick={onClose} className="text-[#555] hover:text-white transition-colors text-xl leading-none">
            ×
          </button>
        </div>
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 border border-[#333] text-[#888] py-2.5 rounded-lg hover:border-[#555] hover:text-white transition-colors text-sm"
            >
              Cancel
            </button>
            <button
